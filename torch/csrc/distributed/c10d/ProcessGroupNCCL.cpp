...
c10::intrusive_ptr<ProcessGroup::Work> ProcessGroupNCCL::gather(
  ...
  if (rank_ == opts.rootRank * inputTensors.size()) {
    check_gpu_tensors(inputTensors);
    std::vector<at::Tensor> outputFlattened = flatten_for_scatter_gather(outputTensors, inputTensors, size_);
    check_gpu_tensors(outputFlattened);
    auto tensor = inputTensors.back();
    RECORD_PARAM_COMMS(  // 参数通信调试信息
        rank_, // rank
        "gather", // colName
        tensor.numel(), // inSize
        tensor.numel() * // outSize
    ...
    return collective(
        inputTensors,
        outputFlattened,
        [&](at::Tensor& input, // 核心部分
            at::Tensor& output,
            ncclComm_t comm,
            at::cuda::CUDAStream& stream) {
          c10::cuda::CUDACachingAllocator::recordStream( // 核心操作
              output.storage().data_ptr(),
              stream);
        ...
        [&](std::vector<at::cuda::CUDAStream>& ncclStreams) {
          // Copy the flattened output tensors to the outputs.
          for (const auto i : c10::irange(outputTensors.size())) {  // 遍历i个设备
            at::cuda::CUDAStreamGuard guard(ncclStreams[i]);
            for (size_t j = 0; j < outputTensors[0].size(); ++j) {
              // See [Sync Streams].
              c10::cuda::CUDACachingAllocator::recordStream(
    ...
    check_gpu_tensors(inputTensors);
    std::vector<at::Tensor> outputFlattened(inputTensors.size(), at::zeros({0})); // 必须要创建一些空的tensor占位
    auto tensor = inputTensors.back();
    RECORD_PARAM_COMMS(
        rank_, // rank
        "gather", // colName
