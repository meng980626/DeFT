...
Reducer::Reducer(
    ...
    : replicas_(std::move(replicas)),
      ...
      forward_bucket_number(-1),
      backward_bucket_number(-1),
      backup_accumulate_times(0),
      algorithm_key(false),
...
void Reducer::store_drop_variable(size_t variable_index) {
  ...
  runGradCallbackForVariable(variable, [&](auto& grad) {
    if (grad.defined()) {
      ...
      if(bucket_index.bucket_index != variable_locators_[variable_index-1].bucket_index){
        if(backup_buckets[bucket_index.bucket_index].empty()){
          std::vector<at::Tensor> tensors;
          tensors.reserve(bucket.replicas.size());
          for (const auto& rep : bucket.replicas) {
            tensors.push_back(rep.contents);
          }
          backup_buckets[bucket_index.bucket_index]=tensors;
          if(bucket_index.bucket_index==0){
            backup_accumulate_times = 1;
          }
        }
        else{
          backup_buckets[bucket_index.bucket_index][replica_index]+=replica.contents;
          if(bucket_index.bucket_index==0){
            backup_accumulate_times++;
          }
        }
      }
    ...
    if (grad.defined()) {
      ...
      if(bucket_index.bucket_index != variable_locators_[variable_index-1].bucket_index){
        if(algorithm_key){
          ...
          it = find(select_bucket_id.begin(), select_bucket_id.end(), bucket_index.bucket_index);
          if (it != select_bucket_id.end()){
            if(backup_buckets[bucket_index.bucket_index].empty()){
              std::vector<at::Tensor> tensors;
              tensors.reserve(bucket.replicas.size());
              for (const auto& rep : bucket.replicas) {
...
c10d::GradBucket Reducer::construct_current_buckets(int index) const {
  auto& bucket = buckets_[index];
  auto variables_for_bucket = get_variables_for_bucket(index, bucket);
  c10d::GradBucket b(
    index,
    buckets_.size(),
...
c10d::GradBucket Reducer::construct_backup_buckets(int index) const {
  auto& bucket = buckets_[index];
  auto variables_for_bucket = get_variables_for_bucket(index, bucket);
  c10d::GradBucket b(
    index,
    buckets_.size(),
    backup_buckets[index][0],
    bucket.replicas[0].offsets,
    bucket.replicas[0].lengths,
    bucket.replicas[0].sizes_vec,
    variables_for_bucket
...
void Reducer::copy_forward_work_result(int bucket_index, at::Tensor bucket_tensor) {
  current_buckets_flag[bucket_index]=true;
  current_buckets[bucket_index][0]=bucket_tensor.clone();
}


std::vector<c10d::GradBucket> Reducer::get_forward_buckets() const {
  std::lock_guard<std::mutex> lock(mutex_);
  
  int pending_buckets = forward_bucket_number;
  std::vector<c10d::GradBucket> forwardBuckets;
  forwardBuckets.reserve(forward_bucket_number);
  
  for(int i = 0;i < accumulate(cut_strategy.begin(),cut_strategy.end(),0);i++){
    if(pending_buckets == 0)
      break;
    if(current_buckets_flag[i] || current_buckets[i].empty())
      continue;
    auto& bucket = buckets_[i];
    auto variables_for_bucket = get_variables_for_bucket(i, bucket);
    forwardBuckets.emplace_back(
        i,
        buckets_.size(),
        current_buckets[i][0],
        bucket.replicas[0].offsets,
  ...
  //cannot select from backup_buckets!!!

  return forwardBuckets;
}
...
void Reducer::autograd_hook(size_t index) {
  ...
  std::vector<int>::iterator it;
	it = find(select_bucket_id.begin(), select_bucket_id.end(), bucket_index.bucket_index);
	if (algorithm_key && it == select_bucket_id.end()){
    //store all
    store_drop_variable(index);
    if (index == variable_locators_.size()-1 && select_bucket_id.empty()){
      require_finalize_ = true;
...
void Reducer::algorithm_init(int f, int b) {
  forward_bucket_number = f;
  backward_bucket_number = b;
  //droprate=
  algorithm_key = true;
  ...
  //for bucket sharding
  cut_strategy.reserve(buckets_.size());
  ...
  float min = variable_size.front();
  float max = variable_size.back();
  float center;
  if(variable_size.size()%2==0){
    center = (variable_size[variable_size.size()/2-1]+variable_size[variable_size.size()/2])/2;
  }else{
    center = variable_size[variable_size.size()/2];
  }
  for(int i = 0;i < grad_size.size();i++){
    // int t = int((float(grad_size[i])/center - 0.5));
    // if(t > 0){
    //   if(t <= droprate - 1)
    //     cut_strategy.push_back(t+1);//rounding
    //   else
    //     cut_strategy.push_back(droprate);
    // }
    // else
  ...
  current_buckets.reserve(buckets_.size());
  backup_buckets.reserve(buckets_.size());
  //lost_buckets_delay_counter.reserve(buckets_.size());
  partid_of_select_bucket.reserve(buckets_.size());
  std::vector<at::Tensor> temp_tensors;
  for (int i = 0;i < buckets_.size();i++){
    current_buckets.push_back(temp_tensors);
    current_buckets_flag.push_back(true);//the initial flags of current queue is diffrent from backup queue
    backup_buckets.push_back(temp_tensors);
    backup_buckets_flag.push_back(false);
    // if (cut_strategy[i] > 1 )
    //   lost_buckets_delay_counter.push_back(-cut_strategy[i]);
    // else
    //   lost_buckets_delay_counter.push_back(100);
    std::vector<int> temp_int_array(cut_strategy[i]);
    partid_of_select_bucket.push_back(temp_int_array);
  }
  //TORCH_WARN(cut_strategy);
}
...
void Reducer::all_reduce_bucket(Bucket& bucket) {
  ...
  if (algorithm_key){
    //TORCH_WARN("before_backup:",next_bucket_,":",at::sum(backup_buckets[next_bucket_][0]));
    GradBucket b = construct_backup_buckets(next_bucket_);
    work_queue.push_back(std::make_pair(next_bucket_,run_comm_hook(b)));
    //backup_buckets_flag[next_bucket_] = true;
  }
  else{
...
void Reducer::mark_bucket_ready(size_t bucket_index) {
  ...
  int i = 0;//use i to represent whether "for" enter the second turn
  for (; next_bucket_ < buckets_.size() && buckets_[next_bucket_].pending == 0;
       next_bucket_++) {
    num_buckets_ready_++;

    std::vector<int>::iterator it;
	  it = find(select_bucket_id.begin(), select_bucket_id.end(), next_bucket_);
    
    if(i>=1 && it == select_bucket_id.end()){
       at::TensorOptions options;
       options = options.dtype(buckets_[next_bucket_].replicas[0].contents.dtype());
       options = options.device(buckets_[next_bucket_].replicas[0].contents.device());
       auto f1 = c10::make_intrusive<c10::ivalue::Future>(c10::TensorType::get());
       f1->markCompleted(at::zeros_like(buckets_[next_bucket_].replicas[0].contents,options));
       buckets_[next_bucket_].future_work = f1;
       continue;
    }
    i++;
...
void Reducer::select_buckets(){
  select_bucket_id.clear();
  for(int i = 0; i < buckets_.size();i++)
    partid_of_select_bucket[i].clear();
  
  int interval = accumulate(cut_strategy.begin(),cut_strategy.end(),0)/backward_bucket_number;
  // TORCH_WARN_ONCE("interval:",interval);

  // TORCH_WARN("current_buckets:",current_buckets_flag);
  // TORCH_WARN("backup_buckets:",backup_buckets_flag);

  int start_point = -1;
  int pending_buckets = backward_bucket_number;
  ...
  for(int i = 0;i < accumulate(cut_strategy.begin(),cut_strategy.end(),0);i++){
    ...
    if(current_buckets_flag[i] || current_buckets[i].empty())
      continue;
    select_bucket_id.push_back(i);
    pending_buckets--;
  }

  //need to select from backup_buckets
  for(int i = 0;i < accumulate(cut_strategy.begin(),cut_strategy.end(),0);i++){
    if(pending_buckets == 0)
      break;
    if(start_point == -1)
      start_point = i;
    if((i - start_point) % interval == 0){
      select_bucket_id.push_back(i);
      pending_buckets--;
    }
  }
  //TORCH_WARN(select_bucket_id);
}
...
void Reducer::prepare_for_backward(
  ...
  if(algorithm_key){
    select_buckets();
    int init_id = 0;
    for (auto iter = select_bucket_id.begin(); iter != select_bucket_id.end(); iter++)
    {
      if (current_buckets_flag[*iter] || current_buckets[*iter].empty()){//select from backup
        init_id = *iter;
        break;
      }
      else{//select from current
        int i = *iter;
        GradBucket b = construct_current_buckets(i);
        //TORCH_WARN("before_current",i,":",at::sum(current_buckets[i][0]));
        work_queue.push_back(std::make_pair(i,run_comm_hook(b)));
        //current_buckets_flag[*iter] = true;
        select_bucket_id.erase(iter--);//pop current id
      }
    }
    if (select_bucket_id.empty()){
      init_id = buckets_.size();
    }
    
    for (int i = 0;i < init_id;i++){
      buckets_[i].pending = 0;
      at::TensorOptions options;
      options = options.dtype(buckets_[i].replicas[0].contents.dtype());
      options = options.device(buckets_[i].replicas[0].contents.device());
      auto f1 = c10::make_intrusive<c10::ivalue::Future>(c10::TensorType::get());
      f1->markCompleted(at::zeros_like(buckets_[i].replicas[0].contents,options));
      buckets_[next_bucket_].future_work = f1;
      next_bucket_++;
    }
    ...
    for (int i = 0;i < buckets_.size();i++){
      std::vector<int>::iterator it;
      it = find(select_bucket_id.begin(), select_bucket_id.end(), i);
      if (it == select_bucket_id.end()){
        buckets_[i].pending = 0;
      }
    }
...
void Reducer::finalize_backward() {
  ...
  if (algorithm_key){
    for (auto& p : work_queue){
      p.second->wait();
      auto future_result = comm_hook_ == nullptr
              ? detail::parseCppCommHookResult(p.second->value())
              : comm_hook_->parseHookResult(p.second->value());
      if (!current_buckets_flag[p.first]){
        //TORCH_WARN("after_current",p.first,":",at::sum(future_result));
        current_buckets[p.first][0].copy_(future_result);
        current_buckets_flag[p.first]=true;
      }
      else{
        //TORCH_WARN("after_backup",p.first,":",at::sum(future_result));
        backup_buckets[p.first][0].copy_(future_result);
        backup_buckets_flag[p.first]=true;
      }
    }
    work_queue.clear();
    ...
    if(update_flag){
      ...
      for (auto& bucket : buckets_) {
        auto& replica = bucket.replicas[0];
        if (bucket.expect_sparse_gradient) {
          //replica.contents.copy_(current_buckets[count_buck][0].div_(backup_accumulate_times));
          replica.contents.copy_(current_buckets[count_buck][0]);
        } else {
          // Reinitialize only `bucket_views_out` with the future_result by
          // following the same logic in `initialize_buckets`.
          //populate_bucket_views_out(replica, current_buckets[count_buck][0].div_(backup_accumulate_times));
          populate_bucket_views_out(replica, current_buckets[count_buck][0]);
        }
        count_buck++;
      ...
      for(int i = 0; i < current_buckets.size();i++){

        current_buckets[i][0] = backup_buckets[i][0].clone();
        current_buckets_flag[i] = backup_buckets_flag[i];
        backup_buckets_flag[i]=false;
        backup_buckets[i].clear();
        //TORCH_WARN("after_update_current",i,":",at::sum(current_buckets[i][0]));
      }
      //TORCH_WARN("update");
    }
    else{
      for (auto& bucket : buckets_) {
        ...
        auto zero_tensor = at::zeros_like(replica.contents,options);
        if (bucket.expect_sparse_gradient) {
          replica.contents.copy_(zero_tensor);
        } else {
        // Reinitialize only `bucket_views_out` with the future_result by
        // following the same logic in `initialize_buckets`.
          populate_bucket_views_out(replica, zero_tensor);
        }
        // Unset allreduce division factor, as it may change in next backwards pass
        // when running with DDP join mode.
        div_factor_ = kUnsetDivFactor;
      ...
      if(current_buckets[0].empty()){
        for(int i = 0; i < current_buckets.size();i++){
          std::vector<at::Tensor> tensors;
          tensors.reserve(backup_buckets[i].size());
          tensors.push_back(backup_buckets[i][0].clone());
          current_buckets[i]=tensors;
          current_buckets_flag[i] = backup_buckets_flag[i];
          backup_buckets_flag[i]=false;
          backup_buckets[i].clear();
          //TORCH_WARN("first",i,":",at::sum(current_buckets[i][0]));
        }
      }
    }
