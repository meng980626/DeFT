...
class TORCH_API Reducer {
 public:
  ...
  void algorithm_init(int f,int b);
  ...
  std::vector<c10d::GradBucket> get_forward_buckets() const;

  void copy_forward_work_result(int bucket_index, at::Tensor bucket_tensor);

  bool get_algorithm_key() const {
    return algorithm_key;
  }

  c10d::GradBucket construct_current_buckets(int index) const;

  c10d::GradBucket construct_backup_buckets(int index) const;
 ...
 protected:
  ...
  int forward_bucket_number;// number of communication buckets in forward 
  int backward_bucket_number;// number of communication buckets in backward
  std::vector<std::vector<at::Tensor>> current_buckets;// current buckets queue for bucket selection
  std::vector<bool> current_buckets_flag;// whether this bucket has been allreduce
  std::vector<std::vector<at::Tensor>> backup_buckets;// backup buckets queue for bucket selection
  std::vector<bool> backup_buckets_flag;// whether this bucket has been allreduce
  int backup_accumulate_times;
  bool algorithm_key; //represents whether the algorithm is already begin
  std::vector<std::pair<int,c10::intrusive_ptr<c10::ivalue::Future>>> work_queue;// record all future_work

  std::vector<int> lost_buckets_delay_counter;// delay accumulation counter for tensor sharding
  std::vector<int> select_bucket_id;//drop which buckets in every step
  std::vector<int> cut_strategy;//how to cut buckets in bucket sharding
  std::vector<std::vector<int>> partid_of_select_bucket;//which part to drop
  ...
  void store_drop_variable(size_t index);
