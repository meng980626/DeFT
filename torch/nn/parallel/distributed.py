...
class DistributedDataParallel(Module, Joinable):
    ...
    def gather_trace_result(self):
        ...
        comptime = 0
        for p in os.listdir('.'):
            if p.startswith("backward-"+str(dist.get_rank())):
                with open(p, 'r') as f:
                    obj = json.load(f)
                    events = obj.get('traceEvents')
                    for event in events:
                        if (event['tid'] == 'stream 20' and event['ph'] == 'X') or event['name'] == 'ncclKernel_AllReduce_TREE_LL_Sum_float(ncclWorkElem)' or event['name'] == 'ncclKernel_AllReduce_RING_LL_Sum_float(ncclWorkElem)' or event['name'] == 'ncclAllReduceTreeLLKernel_sum_f32(ncclColl)' or event['name'] == 'ncclAllGatherRingLLKernel_copy_i8(ncclColl)' or event['name'] == 'ncclAllReduceRingLLKernel_sum_f32(ncclColl)' or event['name'] == 'ncclAllReduceRingLLKernel_sum_f16(ncclColl)':
                            comtime += event['dur']
                        if event.get('args') is not None and event['args'].get('stream') is not None and event['args']['stream'] == 7:
                            comptime += event['dur']
        
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        tensor_comm_time = torch.tensor([comtime], dtype=torch.int64).to(device)
        tensor_comm_time_list = [torch.zeros(1, dtype=torch.int64).to(
            device) for _ in range(dist.group.WORLD.size())] 
        dist.gather(tensor_comm_time, tensor_comm_time_list if dist.get_rank() == 0 else None, 0)

        comm_time_list = []
        
        if dist.get_rank() == 0:
            for i in tensor_comm_time_list:
                comm_time_list.append(i.tolist()[0])
            tensor_comm_time[0] = min(comm_time_list)
        dist.broadcast(tensor_comm_time, 0)

        tensor_comp_time = torch.tensor([comptime], dtype=torch.int64).to(device)
        tensor_comp_time_list = [torch.zeros(1, dtype=torch.int64).to(
            device) for _ in range(dist.group.WORLD.size())]  
        dist.gather(tensor_comp_time, tensor_comp_time_list if dist.get_rank() == 0 else None, 0)

        comp_time_list = []
        
        if dist.get_rank() == 0:
            for i in tensor_comp_time_list:
                comp_time_list.append(i.tolist()[0])
            tensor_comp_time[0] = sum(comp_time_list)/len(comp_time_list)
        dist.broadcast(tensor_comp_time, 0)
        if dist.get_rank() == 0:
            print("comm_time:",tensor_comm_time[0].item()) #commtime
            print("comp_time:",tensor_comp_time[0].item()) #comptime

        return math.ceil(tensor_comm_time[0].item()/tensor_comp_time[0].item())

    def forward(self, *inputs, **kwargs):
        #if self.num_iterations == 2:
            # compression_ratio = self.gather_trace_result()
            # if dist.get_rank() == 0:
            #     print(compression_ratio)
            # if compression_ratio > 1:
            #     self.reducer.set_compression_ratio(compression_ratio)
        #self._match_all_reduce_for_bwd_pass()
        with torch.autograd.profiler.record_function("DistributedDataParallel.forward"):

            torch.distributed.barrier()

            if self.reducer._get_algorithm_key():
                self.forward_buckets = self.reducer._get_forward_buckets()
                self.forward_works.clear()
                for forward_bucket in self.forward_buckets:
                    #print(forward_bucket.index())
                    forward_work = self.reducer._run_comm_hook(forward_bucket)
                    self.forward_works.append(forward_work)
            ...
            if self.reducer._get_algorithm_key():
                # self.forward_buckets
                for i in range(len(self.forward_works)):
                    self.forward_works[i].wait()
                    self.reducer._copy_forward_work_result(self.forward_buckets[i].index(),self.forward_works[i].value()[0])
